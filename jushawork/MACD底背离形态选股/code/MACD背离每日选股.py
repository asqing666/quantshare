...
if __name__ == '__main__':
    ...
    # 获取股票列表
    df_code = get_allstock_code(pro, exchange='', list_status='L', fields='ts_code, name')
    # 只选要00，60开头的，不要st
    ...
    for code in df_code.ts_code:
        print(code)
        df = get_stock_data(pro, code, start, end)
        # 过滤掉上市未满一年的股票
        if (len(df) > 200):
    ...
    print(all_jchadata)
    all_jchadata.to_csv('./MACD选股结果.csv', encoding='gbk', index=False)
